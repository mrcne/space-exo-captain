...
def build_cnn1d(seq_len: int, n_classes: int) -> keras.Model:
    ...
    x = layers.Dropout(0.3)(x)
    outputs = layers.Dense(n_classes, activation="softmax")(x)
    model = keras.Model(inputs, outputs, name="cnn1d_tabular")
    model.compile(optimizer="adam", loss="sparse_categorical_crossentropy", metrics=["accuracy"])
    return model
...
def build_feature_transformer(seq_len: int, n_classes: int, d_model: int = 64, depth: int = 3, heads: int = 4, dropout: float = 0.1) -> keras.Model:
    inputs = keras.Input(shape=(seq_len,), name="features")
    # project scalars to tokens
    x = layers.Reshape((seq_len, 1))(inputs)
    x = layers.Dense(d_model, use_bias=False, name="token_projection")(x)

    # positional (column) embedding
    positions = tf.range(start=0, limit=seq_len, delta=1)
    pos_emb_layer = layers.Embedding(input_dim=seq_len, output_dim=d_model, name="pos_embedding")
    ...
    x = layers.Dropout(0.2)(x)
    outputs = layers.Dense(n_classes, activation="softmax")(x)
    model = keras.Model(inputs, outputs, name="ft_transformer_light")
    model.compile(optimizer="adam", loss="sparse_categorical_crossentropy", metrics=["accuracy"])
    return model
